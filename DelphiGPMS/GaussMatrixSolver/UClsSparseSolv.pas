...
{All these variables are used only by Solve, but have been declared
...
begin {solve1}
//{$IFDEF TRACE}WriteLn('Entering Solve'); {$ENDIF}
  Solve := False;
  PivotStep := 0;
  ...
  if Solved then
  begin
    SetErrorMsg('System already solved', 15, 0, 0);
    goto Fail;
  end;
...
//{$IFDEF TRACE}WriteLn('about to set up row and column counts'); {$ENDIF}
  ...
  begin
    ...
    if (ElmPtr = nil) then
    begin
      SetErrorMsg('Empty Row', 16, Row, 0);
      goto Fail;
    end;
    LastCol := 0;
    while ElmPtr <> nil do
    begin
      ...
      if (Col <= LastCol) then
      begin
        SetErrorMsg('Cols out of Order', 17, Row, Col);
        goto Fail;
      end
      else LastCol := Col;
      Inc(RowCount^[Row]);
...
{$IFDEF DBUG}Assert(Col <= (1 + Neq), '#2134'); {$ENDIF}
    ...
    if (LastCol <> (1 + Neq)) then
    begin
      SetErrorMsg('No RHS', 18, LastCol, 0);
      goto Fail;
    end;
  end; {for row}
  ...
  for Col := 1 to Neq do
  begin
    if (ColCount^[Col] = 0) then
    begin
      SetErrorMsg('Empty Col', 19, Col, 0);
      goto Fail;
    end;
  end;
  ...
  begin
    ...
    {get this if you have RHS but no LHS}
      SetErrorMsg('Row without Variables', 20, Row, 0);
      goto Fail;
    end;
...
//{$IFDEF TRACE}WriteLn('about to scale'); {$ENDIF}
   {for each row, scale all elements, including RHS, so that the largest LHS coefficent is 1.00}
    ...
    {find biggest element}
      ...
      if (Biggest = 0.0) then
      begin
        SetErrorMsg('All-Zero Row', 21, Row, 0);
        goto Fail;
      end;
   ...
   {for each LHS Col, set ColScale^[Col] to largest element in col}
    ...
    begin
      ...
      begin
        Col := ElmPtr^.Column;
        if (Col > Neq) then Break;
        if (Abs(ElmPtr^.Value) > ColScale^[Col]) then
          ColScale^[Col] := Abs(ElmPtr^.Value);
        ElmPtr := ElmPtr^.Next;
      end;
    end; {for row}
    for Col := 1 to Neq do if (ColScale^[Col] = 0.0) then
    begin
      SetErrorMsg('All-Zero Column', 22, Col, 0);
      goto Fail;
    end;
...
{$IFDEF DBUG}Assert(ElmPtr <> nil, '#35'); {$ENDIF}
      ...
      begin
        SetErrorMsg('Two Singles', 23, Row, PivotCol);
        goto Fail;
      end
...
{$IFDEF DBUG}Assert(Row <> 0, '#33'); {$ENDIF}
    while Row <> 0 do
    begin
      if RowCount^[Row] < MinRowCount then
      begin
...
{$IFDEF TRACE}WriteLn('Pivotrow: ', PivotRow, ' Rowcount ', MinRowCount); {$ENDIF}
   {find Biggest Element in Pivot Row}
    Biggest := -1;
    ElmPtr := FirstElm^[PivotRow];
    while (ElmPtr^.Column <= Neq) do
    begin
      if (Abs(ElmPtr^.Value) > Biggest) then Biggest := Abs(ElmPtr^.Value);
      ElmPtr := ElmPtr^.Next;
    end;
    if (Biggest < 0.0) then
    begin {row had no elements}
      SetErrorMsg('Structurally Singular', 26, PivotRow, 0);
      goto Fail;
    end;
    if (Biggest = 0.0) then
    begin
      SetErrorMsg('Numerically Singular', 24, PivotRow, 0);
      goto Fail;
    end;
...
//{$ENDIF}
    ...
    Row := NextActiveRow^[0];
    while ((Row <> 0) and (NumToFind > 0)) do
    begin
    {check that FreeList has enough items for the maximum possible number of insertions}
      if (FreeCount < (Neq - PivotStep)) then
        if not TopUpFreeList then
          goto OutOfSpace;
      ...
      while (ElmPtr^.Column < PivotCol) do
      begin
        PrevPtr := ElmPtr;
        ElmPtr := ElmPtr^.Next;
      end;

      if (ElmPtr^.Column = PivotCol) then
      begin
...
{$IFDEF DBUG}Assert(Next_Tar <> nil, '#99'); {$ENDIF}
            ...
            if (NextTarCol > NextPivotCol) then
            begin
...
{$IFDEF DBUG}Assert(NumToFind = 0, '#66'); {$ENDIF}
  ...
  if not GetMemParaAlign(Pointer(Answer), (1 + Neq) * SizeOf(Double)) then
  begin
    goto OutOfSpace;
  end;
